...
import numpy as np
from timeframes import TsTimeFrames
import pandas as pd
...
from break_adjust import TsRelBreakAdjust
import os
...
def usecase_qcm():
    ...
    times = CCITimes(canname, min_set_days=None,
                     skip_breaktimes=[1, 3]).get_times(gpi=gpi, as_datetime=True)
    ...
    test_kwargs = dict([('test_resample', ('M', 0.3)),
                        ...
                        ('test_check_spearR_sig', [0.7, 0.01])])
    ...
    adjcheck_kwargs = {'adjust_check_fix_temp_coverage': False,
                       ...
                       'adjust_check_pearsR_sig': (0.7, 0.01)}

    ds = TsRelMultiBreak(candidate=ts_full['can'],
    ...
    adjusted = ds.adjust_all(extended_reference=True, **adjfct_kwargs)

    ds.candidate_has_changed()
...
def usecase_hom():
    ...
    test_kwargs = dict([('test_resample', ('M', 0.3)),
                        ...
                        ('alpha', 0.01),
                        ('test_check_min_data', 5),
                        ('test_check_spearR_sig', [0.5, 0.05])])
    ...
    adjcheck_kwargs = {'adjust_check_fix_temp_coverage': False,
                       'adjust_check_min_group_range': 365,
                       'adjust_check_pearsR_sig': (0.3, 0.1)}
...
def usecase_testonly():
    from cci_breaks.cci_timeframes import CCITimes
    from data_read_write.otherfunctions import smart_import
    import shutil

    gpi = 444772
    canname = 'C3S_COMBINED_v201812'
    refname = 'MERRA2'

    times = CCITimes(canname, min_set_days=None,
                     skip_breaktimes=None).get_times(gpi=gpi, as_datetime=True)
    breaktimes, timeframes = times['breaktimes'], times['timeframes']
