...
"""
These functions are used in the tutorials to allow the user to use either artificially generated 
test data with an introduced break or read real ESA CCI SM and MERRA2 model SM data and then introduce a break.
...
def load_test_data(trend=0.01, bias=0.1, breaktime=datetime(2012,7,1), breaksize=(0,1), resample=False, seed=1234):
    ...
    np.random.seed(seed)
    rand_can = pd.Series(index=dt_index, data=np.random.normal(loc=0., scale=0.05, size=3287), name='CAN')
    np.random.seed(seed+1)
    rand_ref = pd.Series(index=dt_index, data=np.random.normal(loc=0., scale=0.05, size=3287), name='REF')
...
def load_real_data(gpi=707393, breaktime=datetime(2012,7,1), breaksize=(0,1), resample=False):
    ...
    can = df.loc['2010-01-01':'2017-12-31', 'ESA CCI SM v04.4 (COMBINED)']
